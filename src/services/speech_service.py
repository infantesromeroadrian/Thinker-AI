...
class SpeechService:
    ...
    def _initialize_microphone(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"🔊 Speech recognition configured with {self.pause_threshold}s silence detection")
    ...
    def record_once(self, 
        ...
        Uses SPEECH_PAUSE_THRESHOLD (3.0s) to detect silence and automatically send response
        """
        ...
        def record_and_recognize():
            try:
                ...
                self.logger.info(f"🎤 Listening... (timeout: {timeout_val}s, phrase limit: {phrase_limit}s, silence detection: {self.pause_threshold}s)")
    ...
    def _continuous_listening_loop(self) -> None:
        ...
        while self.is_listening:
            try:
                ...
                with self.microphone as source:
                    self.logger.debug(f"👂 Listening for speech... (silence detection: {self.pause_threshold}s)")

                    # Listen for speech
                    audio = self.recognizer.listen(
                        source,
                        timeout=self.config.SPEECH_TIMEOUT,
                        phrase_time_limit=self.config.SPEECH_PHRASE_TIME_LIMIT
                    )
